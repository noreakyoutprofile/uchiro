...
## Fixing "ffmpeg and ffprobe not found" / "Requested format is not available"

This app needs three things Railway's auto-detection struggled with: Python, FFmpeg, and Deno (for YouTube's JS challenge). Rather than fight Railway's auto-builder (which switched from "Nixpacks" to a newer "Railpack" system that was misdetecting this project's language), this project now ships a **`Dockerfile`**. Railway always builds from a Dockerfile when one exists at the repo root, so it skips auto-detection entirely and installs exactly what's specified:

- Python 3.12
- FFmpeg (for merging video/audio and MP3 extraction)
- Deno (the JS runtime yt-dlp needs to solve YouTube's challenge and get real formats)

Nothing else to configure — just push this to GitHub and redeploy. Railway should detect the Dockerfile automatically. If it doesn't (e.g. an old cached builder setting from before), go to your service → Settings → and confirm the Builder is set to "Dockerfile."

If you still see either error after redeploying:
1. Check Railway's build logs — you should see `Step ... FROM python:3.12-slim` near the top, confirming it's using the Dockerfile.
2. Confirm the deploy actually picked up the new commit (sometimes a cached build needs a manual redeploy).
3. As a fallback, `/setcookies` (see below) also helps since a logged-in session reduces how often YouTube serves the stripped-down response.
...
2. **In Railway**: New Project → Deploy from GitHub repo → pick this repo.
   Railway detects the `Dockerfile` at the repo root and builds from it automatically — this installs Python, FFmpeg, and Deno exactly as specified, so there's no auto-detection guesswork.
